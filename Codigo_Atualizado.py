...
def obter_tarefas():
    try:
        ...
        return jsonify({'erro': 'Ocorreu um erro ao buscar as tarefas', 'detalhes': str(e)}), 500
...
def adicionar_tarefa():
    try:
        ...
        db.session.rollback()
        return jsonify({'erro': 'Ocorreu um erro ao adicionar a tarefa', 'detalhes': str(e)}), 500
...
def obter_tarefa(id):
    try:
        ...
        return jsonify({'erro': 'Ocorreu um erro ao buscar a tarefa', 'detalhes': str(e)}), 500
...
def atualizar_tarefa(id):
    try:
        ...
        db.session.rollback()
        return jsonify({'erro': 'Ocorreu um erro ao atualizar a tarefa', 'detalhes': str(e)}), 500
...
def remover_tarefa(id):
    try:
        ...
        return jsonify({'mensagem': f'Tarefa com id {id} removida com sucesso'}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({'erro': 'Ocorreu um erro ao remover a tarefa', 'detalhes': str(e)}), 500
